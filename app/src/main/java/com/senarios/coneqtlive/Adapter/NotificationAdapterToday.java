...
public class NotificationAdapterToday extends RecyclerView.Adapter<NotificationAdapterToday.ViewHolder> {
    private List<Today> arrayList = new ArrayList<>();
    private Context context;
    String dateTime;
    String formattedDate;

    public NotificationAdapterToday(List<Today> arrayList, Context context) {
    ...
    @NonNull
    @Override
    public NotificationAdapterToday.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.notication_today_itemlist, parent, false);
        return new NotificationAdapterToday.ViewHolder(view);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (arrayList.get(position).getType().equals("balance_info")) {
            holder.dateTxt.setVisibility(View.VISIBLE);
            holder.titleTxt1.setVisibility(View.VISIBLE);
            holder.itemImageView.setImageResource(R.drawable.ic_notification_dollar);
        }
